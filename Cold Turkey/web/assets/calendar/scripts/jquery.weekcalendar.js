    ...
    return {
      options: {
        ...
        eventHeader: function(calEvent, calendar) {
          ...
          Removed above for Cold Turkey
          */
          // Added below for Cold Turkey
          if (typeof calEvent.id == "string") {
            var breakText = calEvent.break ? '<br><span class="break">' + getBreakText(calEvent.break.toString(), calEvent.blockId.toString()) + '</span>': '<br><span class="break">No breaks</span>';
            // Added above for Cold Turkey, also added " + breakText" below:
            return calEvent.id.split("\\")[0] + breakText;
          } else {
            var breakText = calEvent.break ? '<br><span class="break">' + getBreakText(calEvent.break.toString(), calEvent.blockId.toString()) + '</span>': '<br><span class="break">No breaks</span>';
            // Added above for Cold Turkey, also added " + breakText" below:
            return calendar.weekCalendar('formatTime', calEvent.start) + options.timeSeparator + calendar.weekCalendar('formatTime', calEvent.end) + breakText;
          }
          /*} removed for Cold Turkey*/
        },
      ...
      _create: function() {
        ...
        /* Cold Turkey change: below commented out to get current hour, disable animation to fix issues
      ...
      today: function() {
        ...
        /* Cold Turkey CT Change below - Fake date to prevent daylight savings bug */
      ...
      removeEvent: function(eventId) {
          ...
          self.element.find('.wc-cal-event').each(function() {
            ...
            } else if ($(this).data('calEvent').id > replaceIndex) {
				$(this).data('calEvent').id = $(this).data('calEvent').id - 1;
			}
      ...
      getTimeslotTimes: function(date) {
          ...
          for (var i = 0; i < 720; i++) { /* 720 = 24 * 60 / 5 min, CT change from: for (var i = 0; i < options.timeslotsPerDay; i++) */
		  
            var endMillis = startMillis + 300000; /* 300000 = 5 min, CT change from: = startMillis + options.millisPerTimeslot;*/
      ...
      getCurrentFirstDay: function() {
        ...
        return this._dateFirstDayOfWeek(this.options.date);
        /*CT change above*/
      },
      ...
      _resizeCalendar: function() {
        ...
        if (options && $.isFunction(options.height)) {
          ...
          /* CT Cold Turkey removed because shim makes day titles off-center
      ...
      _renderCalendarHeader: function($calendarContainer) {
        ...
        /* CT Cold Turkey commented out since day headers are off-center with this
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _setupEventCreationForWeekDay: function($weekDay) {
          ...
          $weekDay.mousedown(function(event) {
            ...
            if ($newEvent.length) {
                ...
                currentlyAddingFullDayBlock = false;
      ...
      _drawCurrentHourLine: function() {
        ...
        if (options.firstDayOfWeek == 1) {
          ...
          d = new Date(2020,6,13,actualDate.getHours(),actualDate.getMinutes(),actualDate.getSeconds());
        } else if (options.firstDayOfWeek == 6) {
          fakeDayOffset = (actualDate.getDay() == 6) ? 0 : actualDate.getDay() + 1;
          d = new Date(2020,6,11,actualDate.getHours(),actualDate.getMinutes(),actualDate.getSeconds());
        } else {
          fakeDayOffset = actualDate.getDay();
          d = new Date(2020,6,12,actualDate.getHours(),actualDate.getMinutes(),actualDate.getSeconds());
        }
        d = d.addDays(fakeDayOffset);
        /*d = new Date(),*/
        /* CT Change above */
        ...
        /* the line does not need to be displayed -- removed for cold turkey
        if (businessHours.limitDisplay && d.getHours() > businessHours.end) {
          return;
        } */

        /* then we recreate it -- removed for cold turkey, and paddingStart below
        var paddingStart = businessHours.limitDisplay ? businessHours.start : 0; */
        var nbHours = d.getHours() /*- paddingStart*/ + d.getMinutes() / 60;
        var positionTop = nbHours * this.options.timeslotHeight * this.options.timeslotsPerHour;
      ...
      _renderEvent: function(calEvent, $weekDay) {
		  ...
		  */
		  ...
		  // Below for Cold Turkey 
		  if (typeof calEvent.id == "string") {
			  eventClass = 'wc-cal-event schedule-other-block';
		  } else if (!isNaN(calEvent.id)) {
			  eventClass = 'wc-cal-event';
		  } else {
            ...
            $.each(groupsList, function() {
                ...
                $.each(curGroups, function(groupIndex) {
                  ...
                  $.each(curGroup, function() {
                      ...
                      // Below added for Cold Turkey
                      if (typeof $(this).data('calEvent').illegalOverlap != 'undefined' && $(this).data('calEvent').illegalOverlap) {
                        $(this).addClass("schedule-overlapping-event");
                      } else {
                        $(this).removeClass("schedule-overlapping-event");
                      }
                      if (typeof $(this).data('calEvent').locked != 'undefined' && !$(this).data('calEvent').locked) {
                      if ($(this).width() > 60 && ($(this).height() - $(this).find('.wc-time').height()) > 35) {
                        $(this).find('.wc-title').html('<div class="edit-scheduled-block-cal-event-remove" title="Remove this block" onclick="deleteEvent(this);"><i class="fa fa-times"></i> Remove</div>');
                      } else if ($(this).width() > 10 && ($(this).height() - $(this).find('.wc-time').height()) > 20) {
                        $(this).find('.wc-title').html('<div class="edit-scheduled-block-cal-event-remove-sm" title="Remove this block" onclick="deleteEvent(this);"><i class="fa fa-times"></i></div>');					
                      } else {
                        $(this).find('.wc-title').html('');
                      }
                      } else {
                      $(this).find('.wc-title').html('');
                      }
                      // Above added for Cold Turkey
                  });
                });
            });
          }
      ...
      _groupOverlappingEventElements: function($weekDay) {
          ...
          var groups = [];
          var curGroups = [];
		      var activeBlockTimes = []; // Added for Cold Turkey
          var $curEvent;
          $.each(sortedEvents, function() {
            ...
            // Below added for Cold Turkey
            $curEvent.data('calEvent').illegalOverlap = false;
            if (typeof $curEvent.data('calEvent').id != 'string') {
              for (var i = 0; i < activeBlockTimes.length; i++) {
                if (activeBlockTimes[i][1] > $curEvent.data('calEvent').start.getTime()) {
                  $curEvent.data('calEvent').illegalOverlap = true;
                }
              }
              activeBlockTimes.push([$curEvent.data('calEvent').start.getTime(), $curEvent.data('calEvent').end.getTime()]);
            } 
            // Above added for Cold Turkey
      ...
      _updateEventInCalendar: function(calEvent) {
		  ...
		  // isNaN(X)==false Added for Cold Turkey 
          if (typeof calEvent.id != "string" && !isNaN(calEvent.id)) {
            self.element.find('.wc-cal-event').each(function() {
      ...
      _getEventDurationFromPositionedEventElement: function($weekDay, $calEvent, top) {
          ...
          /* Modified for Cold Turkey (full-day block if user clicked day of week in header) -- added vars, if: original on top*/
          var start;
          var end;
          if (currentlyAddingFullDayBlock) {
            start = new Date($weekDay.data('startDate').getTime() + startOffsetMillis);
            end = new Date(start.addDays(1).getTime());
          } else {
            start = new Date($weekDay.data('startDate').getTime() + startOffsetMillis + Math.round(top / options.timeslotHeight) * options.millisPerTimeslot);
            end = new Date(start.getTime() + Math.ceil($calEvent.height() / options.timeslotHeight) * options.millisPerTimeslot);
          }
      ...
      _addDraggableToCalEvent: function(calEvent, $calEvent) {
        ...
        $calEvent.draggable({
          handle: '.wc-time',
          /*containment: 'div.wc-time-slots', Removed for CT*/ 
      ...
      _scrollToHour: function(hour, animate) {
          ...
          $scrollable.animate({scrollTop: 0}, 0, function() {
            ...
            var maxScroll = (self.options.timeslotHeight * self.options.timeslotsPerHour * 24) - $scrollable.outerHeight();
            var scroll = targetOffset - scrollableOffsetTop - targetOuterHeight;
            ...
            if (animate) {
              $scrollable.animate({scrollTop: scroll}, options.scrollToHourMillis,$.bez([0.29, 0.575, 0.465, 1]));
            }
            else {
      ...
      h: function(date) { return ((date.getHours() % 12 || 12) < 10 ? '' /* CT cold turkey changed '0' to '' to the left */ : '') + (date.getHours() % 12 || 12); },
      ...
      _getHeaderDate: function(date) {
        ...
        /* Cold Turkey change: original below (original includes commented portion) */
        return "<a>" + dayName + "</a>";
